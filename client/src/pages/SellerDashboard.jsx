...
//                   <td className="px-6 py-4 whitespace-nowrap">{order.items.totalAmount}</td>
//                   <td className="px-6 py-4 whitespace-nowrap">{order.quantity}</td>
//                   <td className="px-6 py-4 whitespace-nowrap">{order.total}</td>
//                   <td className="px-6 py-4 whitespace-nowrap">{order.status}</td>
//                 </tr>
...
const SellerDashboard = () => {
  ...
  const fetchWithAuth = async (url) => {
    const token = currentUser.token; // Assuming the token is stored in currentUser object
    ...
    const data = await res.json();
    if (!res.ok) {
      console.error('Failed to fetch', data); // Log the response for debugging
  ...
  useEffect(() => {
    const fetchRecentOrders = async () => {
      try {
        const data = await fetchWithAuth(`/api/orders?sellerId=${currentUser._id}`);
  ...
  useEffect(() => {
    const fetchViews = async () => {
      try {
        const data = await fetchWithAuth(`/api/listing/productviewed?id=${currentUser._id}`);
        setViews(data.views);
      } catch (error) {
        console.error(error);
      }
  ...
  useEffect(() => {
    const fetchAddToCart = async () => {
      try {
        const data = await fetchWithAuth(`/api/listing/addtocart?id=${currentUser._id}`);
        setAddToCart(data.addToCart);
      } catch (error) {
        console.error(error);
      }
  ...
  useEffect(() => {
    const fetchShare = async () => {
      try {
        const data = await fetchWithAuth(`/api/listing/productshared?id=${currentUser._id}`);
        setShares(data.shares);
      } catch (error) {
        console.error(error);
      }
          ...
          <table className="min-w-full bg-white">
            <thead>
              <tr>
                ...
                <th className="px-4 py-2">User Reference</th>
              ...
              {recentOrders.length > 0 ? (
                recentOrders.map((order) => (
                  <tr key={order._id}>
                    ...
                    <td className="border px-4 py-2">{order.userRef}</td>
